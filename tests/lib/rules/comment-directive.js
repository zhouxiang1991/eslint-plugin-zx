...
const linter = new eslint.CLIEngine({
  ...
  rules: {
    ...
    'zx/comment-directive': 'error',
    'zx/no-parsing-error': 'error',
    'zx/no-duplicate-attributes': 'error'
  },
  useEslintrc: false
})
...
describe('comment-directive', () => {
  ...
  describe('eslint-disable/eslint-enable', () => {
    ...
    it('disable specific rules if <!-- eslint-disable vue/no-duplicate-attributes -->', () => {
      ...
      assert.deepEqual(messages[0].ruleId, 'zx/no-parsing-error')
    })
    ...
    it('enable all rules if <!-- eslint-enable -->', () => {
      ...
      assert.deepEqual(messages[1].ruleId, 'zx/no-duplicate-attributes')
    ...
    it('enable specific rules if <!-- eslint-enable vue/no-duplicate-attributes -->', () => {
      ...
      assert.deepEqual(messages.length, 1)
      assert.deepEqual(messages[0].ruleId, 'zx/no-duplicate-attributes')
      assert.deepEqual(messages[0].line, 6)
    })
