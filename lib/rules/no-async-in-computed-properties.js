      ...
      // },
      {
        'ObjectExpression' (node) {
          if (utils.isVueDeclaration(node)) {
            // console.log(node.properties[0].value.properties[0])
            const computedProperties = utils.getComputedProperties(node)

            // console.log()
            computedProperties.forEach(cp => {
              const hasAsync = cp && cp.value && cp.value.parent && cp.value.parent.async
              if (hasAsync) {
                context.report({
                  node: cp.value,
                  message: 'Unexpected {{expressionName}} in "{{propertyName}}" computed property.',
                  data: {
                    expressionName: expressionTypes.async,
                    propertyName: cp.key
                  }
                })
              }
              // console.log('hasAsync', hasAsync, value)
            })
